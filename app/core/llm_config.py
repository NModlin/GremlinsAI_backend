...
class LLMConfig:
    ...
    def _detect_provider(self) -> LLMProvider:
        ...
        # Check for LlamaCpp
        if os.getenv("LLAMACPP_MODEL_PATH"):
            return LLMProvider.LLAMACPP

        # No LLM provider configured - fail fast
        raise RuntimeError(
            "No LLM provider configured. Please set one of the following:\n"
            "- OPENAI_API_KEY for OpenAI\n"
            "- OLLAMA_BASE_URL for Ollama (or ensure Ollama is running on localhost:11434)\n"
            "- USE_HUGGINGFACE=true for Hugging Face\n"
            "- LLAMACPP_MODEL_PATH for LlamaCpp\n"
...
# Global LLM configuration instance - initialized lazily to avoid startup issues
llm_config = None

def get_global_llm_config():
    """Get the global LLM configuration, initializing it if necessary."""
    global llm_config
...
def get_llm():
    ...
    with _llm_cache_lock:
        ...
        if _llm_instance_cache is None or _llm_config_hash != current_hash:
            logger.info(f"Creating new LLM instance (provider: {config.provider}, model: {config.model_name})")
            _llm_instance_cache = create_llm(config)
...
def get_specialized_llm(agent_type: str):
    ...
    agent_configs = {
        ...
        'default': {
            'temperature': get_global_llm_config().temperature,
            'max_tokens': get_global_llm_config().max_tokens,
    ...
    try:
        ...
        logger.debug(f"Parameter binding not supported for {get_global_llm_config().provider}, using base LLM: {e}")
...
def get_agent_config_info(agent_type: str) -> Dict[str, Any]:
    ...
    agent_configs = {
        ...
        'default': {'temperature': get_global_llm_config().temperature, 'max_tokens': get_global_llm_config().max_tokens, 'focus': 'general'}
    }
    ...
    return {
        ...
        "max_tokens": config['max_tokens'],
        "focus": config['focus'],
        "provider": get_global_llm_config().provider.value,
        "base_model": get_global_llm_config().model_name
    }
...
def get_llm_health_status() -> Dict[str, Any]:
    ...
    try:
        ...
        llm_available = test_llm is not None
        ...
        return {
            ...
            "provider": get_global_llm_config().provider.value,
            "model": get_global_llm_config().model_name,
